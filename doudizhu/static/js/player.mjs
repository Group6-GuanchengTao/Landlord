...
export class Player {
    ...
    canPlay(lastTurnPoker, shotPoker) {
        ...
        if (valueA[0] !== valueB[0] && valueA[1] < 1000) {
            return 'The card type is inconsistent with the previous one';
        }

        if (valueA[1] > valueB[1]) {
            return '';
        }
        return 'The card needs to be bigger than the previous one';
    }
...
export class NetPlayer extends Player {
    ...
    initUI(sx, sy) {
        ...
        if (this.seat === 1) {
            this.uiName = this.game.add.text(sx - 40, sy - 80, 'Waiting players', style);
    ...
    updateInfo(uid, name) {
        ...
        if (uid) {
            ...
            this.uiName.text = 'Waiting players ';
        }
    }
