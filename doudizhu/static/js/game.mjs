...
export class Game {
    ...
    create() {
        ...
        const titleBar = this.game.add.text(width / 2, 0, `Room number:${0} Origin Score: 0 Multiple: 0`, {
            font: "22px",
            fill: "#fff",
            align: "center"
        });
        ...
        observer.subscribe('room', function (room) {
            titleBar.text = `Room number:${room.id} Origin Score: ${room.origin} Multiple: ${room.multiple}`;
        });

        // Create the button "Ready"
        const that = this;
        ...
        // Create the button "fight landlord"
    ...
    onmessage(message) {
        ...
        switch (code) {
            ...
            case Protocol.RSP_READY:
                // TODO: Show whether players are ready to play
            ...
            case Protocol.RSP_CALL_SCORE: {
                ...
                const hanzi = ['Not Grab', "Grab the landlord"];
            ...
            case Protocol.RSP_GAME_OVER: {
                ...
                function gameOver() {
                    alert(that.players[that.whoseTurn].isLandlord ? "Landlord win" : "Farmers Win");
    ...
    handleShotPoker(packet) {
        ...
        if (pokers.length === 0) {
            this.players[this.whoseTurn].say("Pass");
        } else {
            let pokersPic = {};
